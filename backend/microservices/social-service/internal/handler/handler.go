...
func NewUserHandler(repo *repository.Neo4jRepository, publisher message.Publisher) *UserHandler {
	return &UserHandler{
		Repo:      repo,
		Publisher: publisher,
...
func (h *UserHandler) HandleCreated(msg *message.Message) error {
	...
	// Use shared model for validation and mapping
	...
	if err := h.Repo.CreatePerson(msg.Context(), user); err != nil {
		h.Logger.ErrorContext(msg.Context(), "failed to create person in neo4j", "error", err)
		return err // Retry
	}

	// Emit user_synced_social event
	syncPayload := struct {
		UserID string `json:"user_id"`
	}{
	...
	if err := h.Publisher.Publish("user_synced_social", syncMsg); err != nil {
		h.Logger.ErrorContext(msg.Context(), "failed to publish user_synced_social event", "error", err)
		return err // Retry
	}

	h.Logger.InfoContext(msg.Context(), "successfully published user_synced_social event", "user_id", payload.UserID)
